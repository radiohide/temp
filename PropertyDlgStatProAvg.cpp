...
BOOL CPropertyDlgStatProAvg::OnInitDialog() 
{
	...
	{
		m_strBlockDesp="统计计算类。将输入的过程数据进行均值计算并以统计数据类型格式输出。";
		((CWnd*)this)->SetWindowText("统计过程数据均值块");
	}
	else if(m_iProOrStatOrSum==1)
	{
		m_strBlockDesp="统计计算类。将输入的统计数据进行均值计算并以统计数据类型格式输出。";
		((CWnd*)this)->SetWindowText("统计统计数据均值块");
	}
	else
	{
		m_strBlockDesp="统计计算类。将输入的统计数据进行求和计算并以统计数据类型格式输出。";
		((CWnd*)this)->SetWindowText("统计统计数据求和块");
	}
	UpdateData(FALSE);
