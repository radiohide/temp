// StatProEnumParaExtBlock.cpp: implementation of the CStatProEnumParaExtBlock class.
//
//////////////////////////////////////////////////////////////////////
...
CStatProEnumParaExtBlock::CStatProEnumParaExtBlock(int iTimeSelfOrInput)
{
	m_dLeft=0.5;//区间左（闭区间）
	...
	SetBlockType(_T( "StatProEnumParaExt" ) );
}

CStatProEnumParaExtBlock::~CStatProEnumParaExtBlock()
{

}

//根据类型新建对象的函数
CCalcBlock* CStatProEnumParaExtBlock::CreateFromString( const CString& strType )
{
	CStatProEnumParaExtBlock* obj = new CStatProEnumParaExtBlock(1);
...
void CStatProEnumParaExtBlock::ConstructBlockInputOutput ()
{
	//初始化时从外部获取时间起始，所以初始时添加两个端口
	AddInputPort("StartTime",VALUE_DOUBLE);
	AddInputPort("EndTime",VALUE_DOUBLE);

	AddInputPort("SpeedLeft",VALUE_DOUBLE);
	AddInputPort("SpeedRight",VALUE_DOUBLE);
}
//初始化计算的函数做特殊的初始化
void CStatProEnumParaExtBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}
//返回属性的逗点字符串
CString CStatProEnumParaExtBlock::GetBlockStrPro()//to do
{
	CString str;
	//str.Format("%s@%s",m_strTagName,m_strTagDesp);
	return str;
}
void CStatProEnumParaExtBlock::SetBlockStrPro(CString str)//设置计算块的字符串属性//to do
{
	//m_strTagName=str.Left(str.Find('@'));
	//m_strTagDesp=str.Right(str.GetLength()-str.ReverseFind('@')-1);
}
void CStatProEnumParaExtBlock::Calc()//进行计算的函数,需要重载
{
	...
	//添加外部数据区间参数方式添加，数据由vestore提供，原采用界面配置方式，该部分为提高参数变换时的适应性，不过会带来组态工具应用层逻辑的混乱。
	//将m_dLeft和m_dRight改为外部输入覆盖原数据
	double dSpeedLeft = 0.5;
	CCalcPort *pInPortObj1 = GetInputPortObj(iInputDataStart);
	pInPortObj1->GetPortUniValue().GetDoubleVal(&dSpeedLeft);
	m_dLeft=dSpeedLeft;
	iInputDataStart = iInputDataStart+1;
	
	double dSpeedRight = 1.5;
	CCalcPort *pInPortObj2 = GetInputPortObj(iInputDataStart);
	pInPortObj2->GetPortUniValue().GetDoubleVal(&dSpeedRight);
	m_dRight=dSpeedRight;
	iInputDataStart = iInputDataStart+1;
...
void CStatProEnumParaExtBlock::statDataTimeLength(CUniValue  &CUV,//统计时长
							 double &dValue,
							 double &dMin,
							 double &dMax,
							 long &lRealSTime,
...
void CStatProEnumParaExtBlock::statDataCount(CUniValue  &CUV,
							 double &dValue,
							 double &dMin,
							 double &dMax,
							 long &lRealSTime,
...
bool CStatProEnumParaExtBlock::isInRange(const double &dValue,const double &dLeft,const double &dRight)
{
	bool bRet=false;
...
void CStatProEnumParaExtBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
void CStatProEnumParaExtBlock::getPropTypeByName(CString strPropName,int &propType,int &propIndex)
{
...
//重载以显示不同的属性对象对话框
void CStatProEnumParaExtBlock::ShowPropertyDialog( CWnd* parent, BOOL show )
{
	CPropertyDlgStatProEnumParaExt dlg;
	...
	if(dlg.DoModal()==IDOK)
	{
		...
		IDArrLeftIn.Add("SpeedLeft");
		IDArrLeftInType.Add(VALUE_DOUBLE);
		IDArrLeftIn.Add("SpeedRight");
		IDArrLeftInType.Add(VALUE_DOUBLE);
...
//重载以将本模块写入到数据库中
void CStatProEnumParaExtBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
...
//复制图形时，需要拷贝属性。
void CStatProEnumParaExtBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	this->strInputArr.Copy(((CStatProEnumParaExtBlock*)pBlock)->strInputArr);
	this->strOutputArr.Copy(((CStatProEnumParaExtBlock*)pBlock)->strOutputArr);
	this->strTagDespArr.Copy(((CStatProEnumParaExtBlock*)pBlock)->strTagDespArr);
	this->strUnitArr.Copy(((CStatProEnumParaExtBlock*)pBlock)->strUnitArr);
	this->iTimeSelfOrInput=((CStatProEnumParaExtBlock*)pBlock)->iTimeSelfOrInput;

	this->m_dLeft=((CStatProEnumParaExtBlock*)pBlock)->m_dLeft;
	this->m_dRight=((CStatProEnumParaExtBlock*)pBlock)->m_dRight;
	this->m_iStatType=((CStatProEnumParaExtBlock*)pBlock)->m_iStatType;
	this->m_iFunType=((CStatProEnumParaExtBlock*)pBlock)->m_iFunType;
	this->m_dGain=((CStatProEnumParaExtBlock*)pBlock)->m_dGain;
}
//将属性放置传输数据中
void CStatProEnumParaExtBlock::SetBlockPropertyToTransferData(
