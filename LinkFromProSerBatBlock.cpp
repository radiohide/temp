// LinkFromProSerBatBlock.cpp: implementation of the CLinkFromProSerBatBlock class.
//
//////////////////////////////////////////////////////////////////////
...
CLinkFromProSerBatBlock::CLinkFromProSerBatBlock()
{
	SetBlockType(_T( "LinkFromProSerBat" ) );//设置类型
...
//构造初始的输入输出个数
void CLinkFromProSerBatBlock::ConstructBlockInputOutput()
{
}
//同时在CCalcBlockControlFactory中，要加入相应的代码
CCalcBlock* CLinkFromProSerBatBlock::CreateFromString( const CString& strType )
{
	CLinkFromProSerBatBlock* obj = new CLinkFromProSerBatBlock;
...
//初始化计算的函数做特殊的初始化
void CLinkFromProSerBatBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}

//计算函数，实现本模块的计算
void CLinkFromProSerBatBlock::Calc() 
{
	//获得输入端口的数据
...
CCalcPort* CLinkFromProSerBatBlock::getRefFromWSByID(CString csID)
{
	CString str;
...
void CLinkFromProSerBatBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString 
...
//重载以显示不同的属性对象对话框
void CLinkFromProSerBatBlock::ShowPropertyDialog( CWnd* parent, BOOL show)
{
	//显示窗口
...
//重载以将本模块写入到数据库中
void CLinkFromProSerBatBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
	//默认固定项
...
//复制图形时，需要拷贝属性。
void CLinkFromProSerBatBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	//this->m_nStatType=((CProcessToStatisBlock*)pBlock)->m_nStatType;
}
//将属性放置传输数据中
void CLinkFromProSerBatBlock::SetBlockPropertyToTransferData(CArray<CBlockPropertyTransferData*,CBlockPropertyTransferData*&> &dataArr)
{
