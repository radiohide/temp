// LinkFromStatSerBatBlock.cpp: implementation of the CLinkFromStatSerBatBlock class.
//
//////////////////////////////////////////////////////////////////////
...
CLinkFromStatSerBatBlock::CLinkFromStatSerBatBlock()
{
	SetBlockType(_T( "LinkFromStatSerBat" ) );//设置类型
...
//构造初始的输入输出个数
void CLinkFromStatSerBatBlock::ConstructBlockInputOutput()
{
}
//同时在CCalcBlockControlFactory中，要加入相应的代码
CCalcBlock* CLinkFromStatSerBatBlock::CreateFromString( const CString& strType )
{
	CLinkFromStatSerBatBlock* obj = new CLinkFromStatSerBatBlock;
...
//初始化计算的函数做特殊的初始化
void CLinkFromStatSerBatBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}

//计算函数，实现本模块的计算
void CLinkFromStatSerBatBlock::Calc() 
{
	//获得输入端口的数据
...
CCalcPort* CLinkFromStatSerBatBlock::getRefFromWSByID(CString csID)
{
	CString str;
...
void CLinkFromStatSerBatBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString 
...
//重载以显示不同的属性对象对话框
void CLinkFromStatSerBatBlock::ShowPropertyDialog( CWnd* parent, BOOL show)
{
	//显示窗口
...
//重载以将本模块写入到数据库中
void CLinkFromStatSerBatBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
	//默认固定项
...
//复制图形时，需要拷贝属性。
void CLinkFromStatSerBatBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	//this->m_nStatType=((CProcessToStatisBlock*)pBlock)->m_nStatType;
}
//将属性放置传输数据中
void CLinkFromStatSerBatBlock::SetBlockPropertyToTransferData(CArray<CBlockPropertyTransferData*,CBlockPropertyTransferData*&> &dataArr)
{
