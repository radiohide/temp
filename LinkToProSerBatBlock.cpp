// LinkToProSerBatBlock.cpp: implementation of the CLinkToProSerBatBlock class.
//
//////////////////////////////////////////////////////////////////////
...
CLinkToProSerBatBlock::CLinkToProSerBatBlock()
{
	SetBlockType(_T( "LinkToProSerBat" ) );//设置类型
...
//构造初始的输入输出个数
void CLinkToProSerBatBlock::ConstructBlockInputOutput()
{
	AddInputPort("Enable",VALUE_DOUBLE);
}
//同时在CCalcBlockControlFactory中，要加入相应的代码
CCalcBlock* CLinkToProSerBatBlock::CreateFromString( const CString& strType )
{
	CLinkToProSerBatBlock* obj = new CLinkToProSerBatBlock;
...
//初始化计算的函数做特殊的初始化
void CLinkToProSerBatBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}

CString CLinkToProSerBatBlock::getRefToWSTagName(int index)
{
...
//计算函数，实现本模块的计算
void CLinkToProSerBatBlock::Calc() 
{
	//获得输入端口的数据
...
void CLinkToProSerBatBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
//重载以显示不同的属性对象对话框
void CLinkToProSerBatBlock::ShowPropertyDialog( CWnd* parent, BOOL show)
{
	//显示窗口
	...
	if(IDOK==dlg.DoModal())
	{
		...
		for(int i=0;i<strInputArr.GetSize();i++)
		{
			...
			IDArrLeftInType.Add(VALUE_RTVALUE_SERIES);//VALUE_RTVALUE_SERIES
		}
...
//重载以将本模块写入到数据库中
void CLinkToProSerBatBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
	//默认固定项
...
//复制图形时，需要拷贝属性。
void CLinkToProSerBatBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	//this->m_nStatType=((CLinkToProBatBlock*)pBlock)->m_nStatType;
}
//将属性放置传输数据中
void CLinkToProSerBatBlock::SetBlockPropertyToTransferData(
