// PropertyDlgRTPointBat.cpp : implementation file
//
...
CPropertyDlgRTPointBat::CPropertyDlgRTPointBat(CWnd* pParent /*=NULL*/)
	: CDialog(CPropertyDlgRTPointBat::IDD, pParent)
{
	//{{AFX_DATA_INIT(CPropertyDlgRTPointBat)
	//}}AFX_DATA_INIT
}


void CPropertyDlgRTPointBat::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CPropertyDlgRTPointBat)
	DDX_Control(pDX, IDC_LIST1, m_List);
	//DDX_Radio(pDX, IDC_RADIO1, m_iSysOrInput);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CPropertyDlgRTPointBat, CDialog)
	//{{AFX_MSG_MAP(CPropertyDlgRTPointBat)
...
/////////////////////////////////////////////////////////////////////////////
// CPropertyDlgRTPointBat message handlers

BOOL CPropertyDlgRTPointBat::OnInitDialog() 
{
...
void CPropertyDlgRTPointBat::initListBoxHead()
{
	UpdateData();
	...
	CString Field[5]={"序号","实时库点名","端口名称","单位","描述"};
	//列表视的表头
...
void CPropertyDlgRTPointBat::initListBoxBody()
{
	int nItem=0;
	CString str;
	for(int i=0;i<this->strRTDBTagArr.GetSize();i++)
	{
		str.Format("%d",nItem+1);
		m_List.InsertItem(nItem,str);
		m_List.SetItemText(nItem,1,strRTDBTagArr[i]);
		m_List.SetItemText(nItem,2,strPortTagArr[i]);
...
void CPropertyDlgRTPointBat::OnButtonAdd() 
{
	// TODO: Add your control notification handler code here
	CDlgVarWithPort dlg;
	if(IDOK==dlg.DoModal())
	{
		strRTDBTagArr.Add(dlg.m_strVarName);
		strPortTagArr.Add(dlg.m_strPortName);
...
void CPropertyDlgRTPointBat::OnButtonEdit() 
{
	// TODO: Add your control notification handler code here
	...
	CDlgVarWithPort dlg;
	dlg.m_strVarName=strRTDBTagArr[nSelectedItem];
	dlg.m_strUnit=strUnitArr[nSelectedItem];
	dlg.m_strDesp=strTagDespArr[nSelectedItem];
	dlg.m_strPortName=strPortTagArr[nSelectedItem];
	if(IDOK==dlg.DoModal())
	{
		strRTDBTagArr[nSelectedItem]=dlg.m_strVarName;
		strUnitArr[nSelectedItem]=dlg.m_strUnit;
		strTagDespArr[nSelectedItem]=dlg.m_strDesp;
		strPortTagArr[nSelectedItem]=dlg.m_strPortName;
		initListBoxHead();
		initListBoxBody();
	}
}

void CPropertyDlgRTPointBat::OnButtonDel() 
{
	// TODO: Add your control notification handler code here
	POSITION pos = m_List.GetFirstSelectedItemPosition();
	...
	{
		strRTDBTagArr.RemoveAt(indexSelArr[i]);
		strUnitArr.RemoveAt(indexSelArr[i]);
		strTagDespArr.RemoveAt(indexSelArr[i]);
		strPortTagArr.RemoveAt(indexSelArr[i]);
	}
	initListBoxHead();
	initListBoxBody();
}

void CPropertyDlgRTPointBat::OnDblclkList1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	OnButtonEdit();
	*pResult = 0;
...
void CPropertyDlgRTPointBat::OnButtonOutput() 
{
	// TODO: Add your control notification handler code here
	CString filename;
	...
	try
	{
		...
		if( database.OpenEx(sSql,CDatabase::noOdbcDialog) )
		{
			...
			for(int i=0;i<strRTDBTagArr.GetSize();i++)
			{
				...
				float persent =(float)((float)i/(float)strRTDBTagArr.GetSize())*100;
...
void CPropertyDlgRTPointBat::OnButtonInput() 
{
	// TODO: Add your control notification handler code here
	...
	{   
		...
		if   (pos   !=   NULL)   
		{
			...
			if((iColCount>3)&&(iRowCount>1))
			{
				...
				strRTDBTagArr.RemoveAll();//点名
				strPortTagArr.RemoveAll();//端口名
				...
				for(int j=2;j<=iRowCount;j++)
				{
					...
					strRTDBTagArr.Add(strValueArr[1]);
					strPortTagArr.Add(strValueArr[2]);
...
void CPropertyDlgRTPointBat::OnRadio1() 
{
	// TODO: Add your control notification handler code here
	
}

void CPropertyDlgRTPointBat::OnOK() 
{
