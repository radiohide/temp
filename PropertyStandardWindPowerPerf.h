/////////////////////////////////////////////////////////////////////////////
// CPropertyStandardWindPowerPerf dialog

class CPropertyStandardWindPowerPerf : public CDialog
{
	// Construction
public:
	CPropertyStandardWindPowerPerf(CWnd* pParent = NULL);   // standard constructor
	
	// Dialog Data
	//{{AFX_DATA(CPropertyStandardWindPowerPerf)
	enum { IDD = IDD_PROPERTYSTANDWINDPOWERPERF_DIALOG };
	CComboBox	m_comboBoxDBSel;
	CString	m_strTableName;
	CString	m_strTurbine;
	int		m_iPowerPorTag;
	double	m_dMinParWindSpeed;
	double	m_dMaxParWindSpeed;
	int		m_iPowerPorTagMin;
	int		m_iPowerPorTagMax;
	//}}AFX_DATA
public:
	CString m_strDBName;//数据库名称
	CArray<int,int> m_ArrSourceRDBTagCon;//连接标志
	CStringArray m_ArrSourceRDBName;//连接名称
	CStringArray m_ArrSourceRDBType;//连接类型
private:
	CStringArray despArr;
	CStringArray typeArr;
public:
	void SetSourceRDBData(CArray<int,int> &m_ArrSourceRDBTagCon,
		CStringArray &m_ArrSourceRDBName,
		CStringArray &m_ArrSourceRDBType
		);
	// Overrides
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CPropertyStandardWindPowerPerf)
protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL
	
	// Implementation
protected:
	
	// Generated message map functions
	//{{AFX_MSG(CPropertyStandardWindPowerPerf)
	virtual BOOL OnInitDialog();
	afx_msg void OnSelchangeComboDbsel();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
