// LinkToStatBatBlock.cpp: implementation of the CLinkToStatBatBlock class.
//
//////////////////////////////////////////////////////////////////////
...
CLinkToStatBatBlock::CLinkToStatBatBlock()
{
	SetBlockType(_T( "LinkToStatBat" ) );//设置类型
...
//构造初始的输入输出个数
void CLinkToStatBatBlock::ConstructBlockInputOutput()
{
	AddInputPort("Enable",VALUE_DOUBLE);
}
//同时在CCalcBlockControlFactory中，要加入相应的代码
CCalcBlock* CLinkToStatBatBlock::CreateFromString( const CString& strType )
{
	CLinkToStatBatBlock* obj = new CLinkToStatBatBlock;
...
//初始化计算的函数做特殊的初始化
void CLinkToStatBatBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}

CString CLinkToStatBatBlock::getRefToWSTagName(int index)
{
...
void CLinkToStatBatBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
//重载以显示不同的属性对象对话框
void CLinkToStatBatBlock::ShowPropertyDialog( CWnd* parent, BOOL show)
{
	//显示窗口
...
//重载以将本模块写入到数据库中
void CLinkToStatBatBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
	//默认固定项
...
//复制图形时，需要拷贝属性。
void CLinkToStatBatBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	//this->m_nStatType=((CLinkToProBatBlock*)pBlock)->m_nStatType;
}
//将属性放置传输数据中
void CLinkToStatBatBlock::SetBlockPropertyToTransferData(
