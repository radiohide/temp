// LinkToStatSerBatBlock.cpp: implementation of the CLinkToStatSerBatBlock class.
//
//////////////////////////////////////////////////////////////////////
...
CLinkToStatSerBatBlock::CLinkToStatSerBatBlock()
{
	SetBlockType(_T( "LinkToStatSerBat" ) );//设置类型
...
//构造初始的输入输出个数
void CLinkToStatSerBatBlock::ConstructBlockInputOutput()
{
	AddInputPort("Enable",VALUE_DOUBLE);
}
//同时在CCalcBlockControlFactory中，要加入相应的代码
CCalcBlock* CLinkToStatSerBatBlock::CreateFromString( const CString& strType )
{
	CLinkToStatSerBatBlock* obj = new CLinkToStatSerBatBlock;
...
//初始化计算的函数做特殊的初始化
void CLinkToStatSerBatBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}

CString CLinkToStatSerBatBlock::getRefToWSTagName(int index)
{
...
//计算函数，实现本模块的计算
void CLinkToStatSerBatBlock::Calc() 
{
	//获得输入端口的数据
...
void CLinkToStatSerBatBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
//重载以显示不同的属性对象对话框
void CLinkToStatSerBatBlock::ShowPropertyDialog( CWnd* parent, BOOL show)
{
	//显示窗口
	...
	if(IDOK==dlg.DoModal())
	{
		...
		for(int i=0;i<strInputArr.GetSize();i++)
		{
			...
			IDArrLeftInType.Add(VALUE_STAT_SERIES);//VALUE_RTVALUE_SERIES
		}
...
//重载以将本模块写入到数据库中
void CLinkToStatSerBatBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
	//默认固定项
...
//复制图形时，需要拷贝属性。
void CLinkToStatSerBatBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	//this->m_nStatType=((CLinkToProBatBlock*)pBlock)->m_nStatType;
}
//将属性放置传输数据中
void CLinkToStatSerBatBlock::SetBlockPropertyToTransferData(
