// DBPointYBlock.cpp: implementation of the DBPointYBlock class.
//
//////////////////////////////////////////////////////////////////////
...
//根据类型新建对象的函数
CCalcBlock* DBPointYBlock::CreateFromString( const CString& strType )
{
	DBPointYBlock* obj = new DBPointYBlock;
...
void DBPointYBlock::ConstructBlockInputOutput ()
{
	AddOutputPort("Out",VALUE_DOUBLE);
}
//初始化计算的函数做特殊的初始化
void DBPointYBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}
//返回属性的逗点字符串
CString DBPointYBlock::GetBlockStrPro()
{
	CString str;
	str.Format("%s@%s",m_strTagName,m_strTagDesp);
	return str;
}
void DBPointYBlock::SetBlockStrPro(CString str)//设置计算块的字符串属性
{
	m_strTagName=str.Left(str.Find('@'));
	m_strTagDesp=str.Right(str.GetLength()-str.ReverseFind('@')-1);
}
//计算函数，实现本模块的计算
void DBPointYBlock::Calc()//进行计算的函数,需要重载
{
	int tagBeCon=0;
	CString strDBType="";
	...
	CUniValue UniVar;
	UniVar.AddData(dRetValue,lRetTime);
	pOutPortObj->SetPortUniValue(UniVar);
	OutputResultToGlobalWS();
}
void DBPointYBlock::DrawShape(CDC* dc, CRect rect,int CutSize)
{
	//dc->Rectangle(rect);
	CPoint roundness;
	////////////////////////////////////////////////
...
BOOL DBPointYBlock::getValueFromDB(_ConnectionPtr &m_pConRDB,
								   CString strDBType,
...
void DBPointYBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
//重载以显示不同的属性对象对话框
void DBPointYBlock::ShowPropertyDialog( CWnd* parent, BOOL show )
{
...
//重载以将本模块写入到数据库中
void DBPointYBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
...
//复制图形时，需要拷贝属性。
void DBPointYBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	this->m_TagTimeOffsetOrInput=((DBPointYBlock*)pBlock)->m_TagTimeOffsetOrInput;
	this->m_lOffset=((DBPointYBlock*)pBlock)->m_lOffset;
	this->m_strTagName=((DBPointYBlock*)pBlock)->m_strTagName;
	this->m_strTagDesp=((DBPointYBlock*)pBlock)->m_strTagDesp;
	this->m_iForwardOrBack=((DBPointYBlock*)pBlock)->m_iForwardOrBack;
	this->m_strTableName=((DBPointYBlock*)pBlock)->m_strTableName;
	this->m_strDBDesp=((DBPointYBlock*)pBlock)->m_strDBDesp;\

	this->m_strFieldVarName=((DBPointYBlock*)pBlock)->m_strFieldVarName;
	this->m_strFieldDateTime=((DBPointYBlock*)pBlock)->m_strFieldDateTime;
	this->m_strFieldState=((DBPointYBlock*)pBlock)->m_strFieldState;
	this->m_strFieldValue=((DBPointYBlock*)pBlock)->m_strFieldValue;
}
//将属性放置传输数据中
void DBPointYBlock::SetBlockPropertyToTransferData(
			CArray<CBlockPropertyTransferData*,CBlockPropertyTransferData*&> &dataArr)
{
