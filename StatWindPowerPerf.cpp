// StatWindPowerPerf.cpp: implementation of the CStatWindPowerPerf class.
//
//////////////////////////////////////////////////////////////////////
...
CStatWindPowerPerf::CStatWindPowerPerf()
{
	...
	SetBlockType(_T( "StatWindPowerPerf" ) );
}
...
//根据类型新建对象的函数
CCalcBlock* CStatWindPowerPerf::CreateFromString( const CString& strType )
{
	CStatWindPowerPerf* obj = new CStatWindPowerPerf();
...
void CStatWindPowerPerf::ConstructBlockInputOutput ()
{
...
//初始化计算的函数做特殊的初始化
void CStatWindPowerPerf::InitCalc()
{
	CCalcBlock::InitCalc();
}
//返回属性的逗点字符串
CString CStatWindPowerPerf::GetBlockStrPro()//to do
{
	CString str;
	//str.Format("%s@%s",m_strTagName,m_strTagDesp);
	return str;
}
void CStatWindPowerPerf::SetBlockStrPro(CString str)//设置计算块的字符串属性//to do
{
	//m_strTagName=str.Left(str.Find('@'));
	//m_strTagDesp=str.Right(str.GetLength()-str.ReverseFind('@')-1);
}
void CStatWindPowerPerf::Calc()//进行计算的函数,需要重载
{
	...
	//debug 输出风速、功率
	//std::string windpowerfile("windpowerfile.txt");
	/*
	CString windpowerfile="windpowerfile.txt";
	std::ofstream offile(windpowerfile,std::iostream::out);
	offile<<"行数："<<WindPowerArr.GetSize()<<std::endl;
	for(int k=0;k<WindPowerArr.GetSize();k++)
	{
		std::ofstream offile(windpowerfile,std::iostream::app);
		WindPowerTag oneTag=WindPowerArr[k];
		CTime sTime(1405071003);
		CString strSTime;
		strSTime=sTime.Format("%Y:%m:%d %H:%M:%S");
		//std::string str = CStringA(strSTime); 
		offile<<(LPCTSTR)strSTime;
		offile<<"  ";
		offile<<oneTag.dWindSpeed;
		offile<<"  ";
		offile<<oneTag.dPower<<std::endl;
	}*/
	//debug end
...
void CStatWindPowerPerf::writeDataToTable(CString strTurbine,CString strTableName,WindPowerTag &oneTag)
{
	int tagBeCon=0;
	CString strDBType="";
	_ConnectionPtr m_pConRDB = this->m_pModel->GetSourceRDBByDesp(m_strDBName,tagBeCon,strDBType);
...
void CStatWindPowerPerf::getBinGroup(
									 CArray<WindPowerTag,WindPowerTag&> &WindPowerArr,
...
void CStatWindPowerPerf::delWindPowerArrByAvgVar(
												 CArray<WindPowerTag,WindPowerTag&> &WindPowerArr)
{
...
void CStatWindPowerPerf::getAvgAndVar(
									  CArray<WindPowerTag,WindPowerTag&> &WindPowerArr,
									  CArray<int,int> &iDiffGroupNums)
{
	for(int i=0;i<iDiffGroupNums.GetSize();i++)
...
void CStatWindPowerPerf::delWindPowerArr(
										 CArray<WindPowerTag,WindPowerTag&> &WindPowerArr)
{
	for(int i=WindPowerArr.GetSize()-1;i>=0;i--)
	{
...
void CStatWindPowerPerf::delTagWindPowerMinMaxArr(
												  CArray<WindPowerTag,WindPowerTag&> &WindPowerArr,
												  CUniValue  &CUV,
												  int iMin,
												  int iMax											
...
void CStatWindPowerPerf::delTagWindPowerArr(
											CArray<WindPowerTag,WindPowerTag&> &WindPowerArr,
											CUniValue  &CUV,
...
BOOL CStatWindPowerPerf::gIsTableExist(_ConnectionPtr &pConnection, CString strTableName)
{
	CString strSQL;
...
void CStatWindPowerPerf::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
//重载以显示不同的属性对象对话框
void CStatWindPowerPerf::ShowPropertyDialog( CWnd* parent, BOOL show )
{
...
//重载以将本模块写入到数据库中
void CStatWindPowerPerf::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
...
//复制图形时，需要拷贝属性。
void CStatWindPowerPerf::CopyPropertyData(CCalcBlock* pBlock)
{
	this->m_strDBName=((CStatWindPowerPerf*)pBlock)->m_strDBName;//数据库名称
	this->m_strTableName=((CStatWindPowerPerf*)pBlock)->m_strTableName;
	this->m_strTurbine=((CStatWindPowerPerf*)pBlock)->m_strTurbine;
	this->m_iPowerPorTagMin=((CStatWindPowerPerf*)pBlock)->m_iPowerPorTagMin;
	this->m_iPowerPorTagMax=((CStatWindPowerPerf*)pBlock)->m_iPowerPorTagMax;
	this->m_dPowerLimit=((CStatWindPowerPerf*)pBlock)->m_dPowerLimit;
	this->m_dMinParWindSpeed=((CStatWindPowerPerf*)pBlock)->m_dMinParWindSpeed;
	this->m_dMaxParWindSpeed=((CStatWindPowerPerf*)pBlock)->m_dMaxParWindSpeed;

}
//将属性放置传输数据中
void CStatWindPowerPerf::SetBlockPropertyToTransferData(
