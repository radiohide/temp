...
{
	return;
	/*//该函数在release版本下会出现报错，考虑无编辑回撤功能，故注释了该函数。
	CString strLog;
	strLog.Format("CDiagramEntityContainer::Snapshot--start");
	...
	strLog.Format("CDiagramEntityContainer::Snapshot--2");
	g_Log.WriteFileData(strLog);
	...
	strLog.Format("CDiagramEntityContainer::Snapshot--3");
	g_Log.WriteFileData(strLog);
	if( undo )
	{
		...
		strLog.Format("CDiagramEntityContainer::Snapshot--4");
		g_Log.WriteFileData(strLog);
		// Save all objects
		...
		strLog.Format("CDiagramEntityContainer::Snapshot--5");
		g_Log.WriteFileData(strLog);
		// Add to undo stack
		m_undo.Add( undo );

		strLog.Format("CDiagramEntityContainer::Snapshot--6");
		g_Log.WriteFileData(strLog);

	}
	strLog.Format("CDiagramEntityContainer::Snapshot--end");
	g_Log.WriteFileData(strLog);
	*/
}
