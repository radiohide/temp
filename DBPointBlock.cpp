// DBPointBlock.cpp: implementation of the CDBPointBlock class.
//
//////////////////////////////////////////////////////////////////////
...
//根据类型新建对象的函数
CCalcBlock* CDBPointBlock::CreateFromString( const CString& strType )
{
	CDBPointBlock* obj = new CDBPointBlock;
...
void CDBPointBlock::ConstructBlockInputOutput ()
{
	AddOutputPort("Out",VALUE_DOUBLE);
}
//初始化计算的函数做特殊的初始化
void CDBPointBlock::InitCalc()
{
	CCalcBlock::InitCalc();
}
//返回属性的逗点字符串
CString CDBPointBlock::GetBlockStrPro()
{
	CString str;
	str.Format("%s@%s",m_strTagName,m_strTagDesp);
	return str;
}
void CDBPointBlock::SetBlockStrPro(CString str)//设置计算块的字符串属性
{
	m_strTagName=str.Left(str.Find('@'));
	m_strTagDesp=str.Right(str.GetLength()-str.ReverseFind('@')-1);
}
//计算函数，实现本模块的计算
void CDBPointBlock::Calc()//进行计算的函数,需要重载
{
	int tagBeCon=0;
	CString strDBType="";
	...
	if(m_strTagName!="")
	{
		getValueFromDB(m_pConRDB,strDBType,m_strTableName,m_strTagName,timeflag,m_iForwardOrBack,
			dRetValue,lRetTime,iRetState);
	}
	CUniValue UniVar;
	UniVar.AddData(dRetValue);
	pOutPortObj->SetPortUniValue(UniVar);
	OutputResultToGlobalWS();
}
void CDBPointBlock::DrawShape(CDC* dc, CRect rect,int CutSize)
{
	//dc->Rectangle(rect);
	CPoint roundness;
	////////////////////////////////////////////////
...
BOOL CDBPointBlock::getValueFromDB(_ConnectionPtr &m_pConRDB,
								   CString strDBType,
...
void CDBPointBlock::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
//重载以显示不同的属性对象对话框
void CDBPointBlock::ShowPropertyDialog( CWnd* parent, BOOL show )
{
...
//重载以将本模块写入到数据库中
void CDBPointBlock::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
...
//复制图形时，需要拷贝属性。
void CDBPointBlock::CopyPropertyData(CCalcBlock* pBlock)
{
	this->m_TagTimeOffsetOrInput=((CDBPointBlock*)pBlock)->m_TagTimeOffsetOrInput;
	this->m_lOffset=((CDBPointBlock*)pBlock)->m_lOffset;
	this->m_strTagName=((CDBPointBlock*)pBlock)->m_strTagName;
	this->m_strTagDesp=((CDBPointBlock*)pBlock)->m_strTagDesp;
	this->m_iForwardOrBack=((CDBPointBlock*)pBlock)->m_iForwardOrBack;
	this->m_strTableName=((CDBPointBlock*)pBlock)->m_strTableName;
	this->m_strDBDesp=((CDBPointBlock*)pBlock)->m_strDBDesp;\

	this->m_strFieldVarName=((CDBPointBlock*)pBlock)->m_strFieldVarName;
	this->m_strFieldDateTime=((CDBPointBlock*)pBlock)->m_strFieldDateTime;
	this->m_strFieldState=((CDBPointBlock*)pBlock)->m_strFieldState;
	this->m_strFieldValue=((CDBPointBlock*)pBlock)->m_strFieldValue;
}
//将属性放置传输数据中
void CDBPointBlock::SetBlockPropertyToTransferData(
			CArray<CBlockPropertyTransferData*,CBlockPropertyTransferData*&> &dataArr)
{
