...
void CStatWindPowerPerfParaExt::SetBlockStrPro(CString str)//设置计算块的字符串属性//to do
{
	//m_strTagName=str.Left(str.Find('@'));
	//m_strTagDesp=str.Right(str.GetLength()-str.ReverseFind('@')-1);
}
void CStatWindPowerPerfParaExt::Calc()//进行计算的函数,需要重载
{
	...
	//删除偏差数据  2014.7.26 rao 处理数据库部分数据缺失造成的误差问题
...
void CStatWindPowerPerfParaExt::delTimeOffsetOutWindPowerArr(CArray<WindPowerTag,WindPowerTag&> &WindPowerArr)
{
	for(int i=WindPowerArr.GetSize()-1;i>=0;i--)
	{
		...
		if (lcurTimeOffset<(0-lTimeOffset) && lcurTimeOffset>lTimeOffset)//风速功率时间戳在vestore一样，此处只判断一次
		{
			WindPowerArr[i].bDel=true;//添加删除标志
		}
	}
}
...
{
	this->m_strDBName=((CStatWindPowerPerfParaExt*)pBlock)->m_strDBName;//数据库名称
