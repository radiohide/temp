...
void CAddBlock::Calc()//进行计算的函数,需要重载
{
	...
	CString pPortObjs= pPortObj->GetPortLinkID();
	...
	for(int k=inNum-1;k>0;k--)
	{
		...
		if('+'==m_strInputEdit.GetAt(k-1))
		{
			sum+=inValue;
		}
		else if('-'==m_strInputEdit.GetAt(k-1))
		{
			sum-=inValue;
		}
...
void CAddBlock::buildInputPort(CStringArray &IDArrLeftOut)
{
	...
	//添加使能端2014-4-9 rao
	IDArrLeftIn.Add(IDArrLeftOut[0]);
