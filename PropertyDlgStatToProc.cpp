// PropertyDlgStatToProc.cpp : implementation file
//
...
CPropertyDlgStatToProc::CPropertyDlgStatToProc(CWnd* pParent /*=NULL*/)
	: CDialog(CPropertyDlgStatToProc::IDD, pParent)
{
	//{{AFX_DATA_INIT(CPropertyDlgStatToProc)
	//}}AFX_DATA_INIT
}


void CPropertyDlgStatToProc::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CPropertyDlgStatToProc)
	DDX_Control(pDX, IDC_LIST, m_List);
	DDX_Text(pDX, IDC_EDIT_PROC_TYPE, m_nProcType);
	//}}AFX_DATA_MAP
}

BOOL CPropertyDlgStatToProc::OnInitDialog() 
{
	UpdateData(FALSE);

	//单选初始化化 部分初始化在窗口类创建代码处完成
	switch(iTimeSelect)
	{
	case 0:
		{
			((CButton*)GetDlgItem(IDC_RADIO_ISTIME))->SetCheck(1);
		}
		break;

	case 1:
		{
			((CButton*)GetDlgItem(IDC_RADIO_IETIME))->SetCheck(1);
		}
		break;

	case 2:
		{
			((CButton*)GetDlgItem(IDC_RADIO_IINSERTTIME))->SetCheck(1);
		}
		break;
...
void CPropertyDlgStatToProc::initListBoxBody()
{
	int nItem=0;
	CString str;
...
void CPropertyDlgStatToProc::initListBoxHead()
{
	UpdateData();
...
BEGIN_MESSAGE_MAP(CPropertyDlgStatToProc, CDialog)
	//{{AFX_MSG_MAP(CPropertyDlgStatToProc)
...
/////////////////////////////////////////////////////////////////////////////
// CPropertyDlgStatToProc message handlers

void CPropertyDlgStatToProc::OnOK() 
{
	// TODO: Add extra validation here
	UpdateData();
	if(((CButton*)GetDlgItem(IDC_RADIO_IETIME))->GetCheck())
	{
		iTimeSelect = 1;
	}

	if(((CButton*)GetDlgItem(IDC_RADIO_ISTIME))->GetCheck())
	{
		iTimeSelect = 0;
	}

	if(((CButton*)GetDlgItem(IDC_RADIO_IINSERTTIME))->GetCheck())
	{
		iTimeSelect = 2;
	}
...
void CPropertyDlgStatToProc::OnButtonAdd() 
{
...
void CPropertyDlgStatToProc::OnButtonEdit() 
{
	// TODO: Add your control notification handler code here
...
void CPropertyDlgStatToProc::OnButtonDel() 
{
	// TODO: Add your control notification handler code here
	POSITION pos = m_List.GetFirstSelectedItemPosition();
...
void CPropertyDlgStatToProc::OnButtonInput() 
{
	// TODO: Add your control notification handler code here
...
void CPropertyDlgStatToProc::OnButtonOutput() 
{
	// TODO: Add your control notification handler code here
	CString filename;
