// StatLimitLostPowerParaExtBlock15.cpp: implementation of the CStatLimitLostPowerParaExtBlock15 class.
//
//////////////////////////////////////////////////////////////////////
...
CStatLimitLostPowerParaExtBlock15::CStatLimitLostPowerParaExtBlock15()
{
	dMaxPower=3150000;
	this->iTimeSelfOrInput=1;
	ConstructBlockInputOutput();
	SetBlockType(_T( "LimitLostPowerParaExt15" ));
}
...
//根据类型新建对象的函数
CCalcBlock* CStatLimitLostPowerParaExtBlock15::CreateFromString( const CString& strType )
{
	CStatLimitLostPowerParaExtBlock15* obj = new CStatLimitLostPowerParaExtBlock15();
...
void CStatLimitLostPowerParaExtBlock15::ConstructBlockInputOutput ()
{
	...
	AddInputPort("AGCConstant",VALUE_DOUBLE);
	

	AddOutputPort("OutPut",VALUE_STAT);
}
//初始化计算的函数做特殊的初始化
void CStatLimitLostPowerParaExtBlock15::InitCalc()
{
	CCalcBlock::InitCalc();
}

//返回属性的逗点字符串
CString CStatLimitLostPowerParaExtBlock15::GetBlockStrPro()//to do
{
	CString str;
	//str.Format("%s@%s",m_strTagName,m_strTagDesp);
	return str;
}
void CStatLimitLostPowerParaExtBlock15::SetBlockStrPro(CString str)//设置计算块的字符串属性//to do
{
	//m_strTagName=str.Left(str.Find('@'));
	//m_strTagDesp=str.Right(str.GetLength()-str.ReverseFind('@')-1);
}
void CStatLimitLostPowerParaExtBlock15::Calc()//进行计算的函数,需要重载
{
		//参数外置代码
	//判断是否存在外置风率表，没有拷贝一份
	...
	//zoucheng add 2015-3-10
	double dPowerLimitExt = 3150000;
	CCalcPort *pInPortObjPower = GetInputPortObj(iInputDataStart+1);
	pInPortObjPower->GetPortUniValue().GetDoubleVal(&dPowerLimitExt);
	dMaxPower=dPowerLimitExt;
...
double CStatLimitLostPowerParaExtBlock15::getLimitPowerByTime(CUniValue &CUV,long lParTime)
{
	double dPower=0;
...
*/
double CStatLimitLostPowerParaExtBlock15::getPowerFromWindSpeed(double dWindSpeed)
{
	double dPower=0;
...
void CStatLimitLostPowerParaExtBlock15::SetPropValue(CString strPropName,CString strItem1,CString strItem2,CString strItem3,CString strItem4,CString strItem5)
{
...
void CStatLimitLostPowerParaExtBlock15::getPropTypeByName(CString strPropName,int &propType,int &propIndex)
{
...
//重载以显示不同的属性对象对话框
void CStatLimitLostPowerParaExtBlock15::ShowPropertyDialog( CWnd* parent, BOOL show )
{
	...
	if(dlg.DoModal()==IDOK)
	{
		...
		IDArrLeftIn.Add("AGCConstant");IDArrLeftInType.Add(VALUE_DOUBLE);
...
//重载以将本模块写入到数据库中
void CStatLimitLostPowerParaExtBlock15::PutPropertyToRecord(_RecordsetPtr& pRecord)
{
...
//复制图形时，需要拷贝属性。
void CStatLimitLostPowerParaExtBlock15::CopyPropertyData(CCalcBlock* pBlock)
{
	this->iTimeSelfOrInput=((CStatLimitLostPowerParaExtBlock15*)pBlock)->iTimeSelfOrInput;
	this->dMaxPower=((CStatLimitLostPowerParaExtBlock15*)pBlock)->dMaxPower;
	this->dWindSpeedArr.Copy(((CStatLimitLostPowerParaExtBlock15*)pBlock)->dWindSpeedArr);
	this->dPowerArr.Copy(((CStatLimitLostPowerParaExtBlock15*)pBlock)->dPowerArr);
}
//将属性放置传输数据中
void CStatLimitLostPowerParaExtBlock15::SetBlockPropertyToTransferData(
			CArray<CBlockPropertyTransferData*,CBlockPropertyTransferData*&> &dataArr)
{

}

char* CStatLimitLostPowerParaExtBlock15::GetParaExtName()
{
...
void CStatLimitLostPowerParaExtBlock15::GetStateLostPara(const char* FileName)
{	
